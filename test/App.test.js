module.exports = {
	...
	"Testing Navbar in applicant page": function (browser) {
		browser.assert
			.textContains("nav > ul > li:nth-of-type(1) > a", "Apply Course")
	...
	"Testing Status page of applicants": function (browser) {
		browser.click("nav > ul > li:nth-of-type(2) > a").assert.urlEquals("http://localhost:8000/status");
		const courseDetails = [
	...
	"Testing Navbar in admin page": function (browser) {
		browser.assert
			.textContains("nav > ul > li:nth-of-type(1) > a", "Applications")
	...
	"Testing Applications page of admin": function (browser) {
		...
		browser.assert.not
			.elementPresent("#approvedApplicationsTable")
			//Rejecting course 2
		...
		const approvedCourses = [["1", "Civil Engineering", "John Doe", "john@gmail.com", "95"]];
		for (let i = 1; i <= 1; i++) {
			for (let j = 2; j <= 6; j++) {
	...
	"Testing AddSeats page of admin": function (browser) {
		browser
			.click("nav > ul > li:nth-of-type(2) > a")
